...
class PredictionService:
    ...
    def _load_artifacts(self):
        ...
        try:
            ...
            print(
                f"Lỗi: Không tìm thấy file cần thiết trong thư mục '{MODELS_DIR}'. {e}"
            )
            print(
                "Vui lòng đảm bảo các file 'best_lgbm_regressor.joblib', 'scaler.joblib', 'feature_columns.joblib' có trong thư mục 'models'."
            )
    ...
    def _extract_processor_brand(self, processor):
        ...
        if "Snapdragon" in processor:
            ...
            "MediaTek" in processor or "Dimensity" in processor or "Helio" in processor
        ):
            return "MediaTek"
    ...
    def preprocess_data(self, raw_data: dict) -> pd.DataFrame:
        ...
        df_new["Launched Year"] = pd.to_datetime(
            df_new["Launched Year"], format="%Y"
        ).dt.year.astype("int")

        # 2. Xử lý 'Mobile Weight (g)'
        df_new["Mobile Weight (g)"] = (
            df_new["Mobile Weight"].str.replace("g", "").astype(float)
        )
        ...
        df_new["Front Camera (MP)"] = df_new["Front Camera"].apply(
            self._clean_front_camera
        )
        ...
        df_new[temp_cols] = df_new["Back Camera"].apply(
            lambda x: pd.Series(self._clean_back_camera(x))
        )

        # 6. Xử lý 'Battery Capacity (mAh)'
        df_new["Battery Capacity (mAh)"] = (
            df_new["Battery Capacity"]
            .str.replace("mAh", "")
            .str.replace(",", "")
            .astype(int)
        )
        ...
        df_new["Screen Size (inches)"] = (
            df_new["Screen Size"].apply(lambda x: x.split("inches")[0]).astype(float)
        )
        ...
        df_new["Processor_Brand"] = df_new["Processor"].apply(
            self._extract_processor_brand
        )
        ...
        df_ohe_new = pd.get_dummies(
            df_new[categorical_cols_for_ohe].copy(), drop_first=False
        )

        # Tạo DataFrame cuối cùng với tất cả các cột đặc trưng đã huấn luyện
        final_input_df = pd.DataFrame(
            0, index=[0], columns=self.trained_feature_columns
        )
    ...
    def predict_new_phone_price(self, new_phone_data_raw: dict) -> float:
        ...
        """
        if (
            self.best_lgbm_model is None
            or self.scaler is None
            or self.trained_feature_columns is None
        ):
            raise Exception(
                "Mô hình, scaler hoặc cột đặc trưng chưa được tải. Vui lòng kiểm tra log."
            )
